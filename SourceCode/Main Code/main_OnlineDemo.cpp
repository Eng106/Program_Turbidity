...
const uint32_t DemoModeDelay = 3000; // msec before Initialization, used for online simulation only
...
\----------------------------------------*/
BaseType_t MBCore;   // Core 0 [ModBus Loop Core]
...
{ 
  ...
  Sensor.port.begin(9600, SerialConfig::SERIAL_8N1);
  delay(1000);
...
{
  Sensor.read(10, host); // check if Sensor available, wait for (1..10 second)
...
String SensorObject::read(u_int wait_t_seconds, HardwareSerial* host)
{
  ...
  {    
    ...
    if (valValid)
    {
      host->printf("Sensor Read successfully\r\n"); yield();
      host->printf("txtResponse: \"%s\" \r\n", &txtResponse); yield();
